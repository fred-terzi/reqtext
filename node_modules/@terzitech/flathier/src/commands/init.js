...
export default async function init(fileName = 'FlatHierFormat', customExt = '.fhr') {
  ...
  const folderPath      = path.join(root, `.${customExt}`);
  const destinationPath = path.join(folderPath, `template.${customExt}.json`);
  const mainFileName    = fileName.endsWith(`.${customExt}`)
    ? fileName
    : `${fileName}.${customExt}.json`;

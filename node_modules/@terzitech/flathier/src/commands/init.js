 ...
 */
export default async function init(fileName = 'FlatHierFormat', customExt = '.fhr', customID = 'unqiue_id') {
  ...
  // Store customExt in customExtStore.json for future use
  try {
    const storeData = await fs.readFile(customExtStorePath, 'utf-8');
    let storeArray = JSON.parse(storeData);
    if (Array.isArray(storeArray) && storeArray.length > 0) {
      storeArray[0].customExt = customExt;
    } else {
      storeArray = [{ customExt }];
    }
    await fs.writeFile(customExtStorePath, JSON.stringify(storeArray, null, 2));
